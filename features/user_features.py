...
def create_user_feature_value(expected_date_tup, samples, users_on_off):
    ...
    for index in range(sample_cnt):
        ...
        if (user_id in user_calculated_set):
            ...
            if (index % 10000 == 0):
                print("calculating User feature values, %d / %d handled\r" % (index, sample_cnt), end="")
            continue
...
def create_user_online_matrix(samples):
    ...
    for index in range(sample_cnt):
        ...
        if (user_id in user_feature_values_dict):
            ...
            if (index % 10000 == 0):
                print("creating feature matrix, %d / %d handled\r" % (index, sample_cnt), end="")       
            continue            
        ...
        if (index % 1000 == 0):
            print("%s creating user feature matrix, %d / %d handled\r" % (getCurrentTime(), index, sample_cnt), end="")

        user_feature_values_dict[user_id] = user_features_matrix[index] 

    return user_features_matrix[:, 0:feature_idx]    
...
def create_user_on_off_matrix(samples, on_off):
    ...
    for index in range(sample_cnt):
        ...
        # 用户从领取优惠券到使用之间的平均天数
        # 这里先计算总共的天数，然后再除以使用次数
        user_features_matrix[index, feature_idx] = user_coupon_days_diff_dict[user_id][ONLINE] / user_get_coupon_cnt_total
...
def create_user_feature_matrix(expected_date_tup, samples):
    ...
    for index in range(len(samples)):
        ...
        # 用户领取优惠券的次数以及线上线下占总领取次数的比例
        user_features_matrix[index, feature_idx] = user_get_coupon_cnt_dict[user_id][ONLINE]; feature_idx += 1
        user_features_matrix[index, feature_idx] = user_get_coupon_cnt_dict[user_id][OFFLINE]; feature_idx += 1
        # 比例
        user_get_coupon_cnt_total = user_get_coupon_cnt_dict[user_id][ONLINE] + user_get_coupon_cnt_dict[user_id][OFFLINE] + 1
        user_features_matrix[index, feature_idx] = user_get_coupon_cnt_dict[user_id][ONLINE]/user_get_coupon_cnt_total; feature_idx += 1
        user_features_matrix[index, feature_idx] = user_get_coupon_cnt_dict[user_id][OFFLINE]/user_get_coupon_cnt_total; feature_idx += 1
        
        
        # 用户从领取优惠券到使用之间的平均天数
        # 这里先计算总共的天数，然后再除以使用次数
        user_features_matrix[index, feature_idx] = user_coupon_days_diff_dict[user_id][ONLINE] / user_get_coupon_cnt_total
        user_features_matrix[index, feature_idx] = user_coupon_days_diff_dict[user_id][OFFLINE] / user_get_coupon_cnt_total

        # 用户使用优惠券消费的次数, 线上线下各占总使用次数的比例以及从领取优惠券到使用之间的平均天数
        user_features_matrix[index, feature_idx] = len(user_buy_cnt_with_coupon_dict[user_id][ONLINE]); feature_idx += 1
        user_features_matrix[index, feature_idx] = len(user_buy_cnt_with_coupon_dict[user_id][OFFLINE]); feature_idx += 1
        # 比例
        user_buy_with_coupon_total = len(user_buy_cnt_with_coupon_dict[user_id][ONLINE]) + len(user_buy_cnt_with_coupon_dict[user_id][OFFLINE]) + 1
        user_features_matrix[index, feature_idx] = len(user_buy_cnt_with_coupon_dict[user_id][ONLINE]); feature_idx += 1
        user_features_matrix[index, feature_idx] = len(user_buy_cnt_with_coupon_dict[user_id][OFFLINE])/user_buy_with_coupon_total; feature_idx += 1
        # 平均天数
        user_features_matrix[index, feature_idx] = \
            round(np.sum(user_buy_cnt_with_coupon_dict[user_id][ONLINE])/
                  (len(user_buy_cnt_with_coupon_dict[user_id][ONLINE]) + 1), 1); feature_idx += 1
        user_features_matrix[index, feature_idx] = \
            round(np.sum(user_buy_cnt_with_coupon_dict[user_id][OFFLINE])/
                  (len(user_buy_cnt_with_coupon_dict[user_id][OFFLINE]) + 1), 1); feature_idx += 1

        # 用户没有使用优惠券消费的次数
        user_features_matrix[index, feature_idx] = user_buy_cnt_without_coupon_dict[user_id][ONLINE]; feature_idx += 1
        user_features_matrix[index, feature_idx] = user_buy_cnt_without_coupon_dict[user_id][OFFLINE]; feature_idx += 1
 
        # 用户使用 【0， 50】， 【51， 200】，【201， 500】， >500 的优惠券占使用优惠券的次数的比例
        user_features_matrix[index, feature_idx:feature_idx+4] = \
            user_coupon_fraction_dict[user_id][ONLINE][0]/ (len(user_buy_cnt_with_coupon_dict[user_id][ONLINE]) + 1); feature_idx += 4
        user_features_matrix[index, feature_idx:feature_idx+4] = \
            user_coupon_fraction_dict[user_id][OFFLINE][0]/ (len(user_buy_cnt_with_coupon_dict[user_id][OFFLINE]) + 1); feature_idx += 4

        # 用户使用优惠券各个折扣率出现的次数  <=0.5, (0.5, 0.7], (0.7, 0.9], > 0.9
        user_features_matrix[index, feature_idx:feature_idx+4] = user_coupon_fraction_dict[user_id][ONLINE][1]; feature_idx += 4
        user_features_matrix[index, feature_idx:feature_idx+4] = user_coupon_fraction_dict[user_id][OFFLINE][1]; feature_idx += 4

        # 用户使用优惠券的平均折扣率 
        # sum(用户使用优惠券各个折扣率出现的次数 * 折扣率中值) /  用户使用优惠的次数
        discount_rate_mid = np.array([0.25, 0.6, 0.8, 0.95])
        user_features_matrix[index, feature_idx] =  \
            np.sum(user_coupon_fraction_dict[user_id][ONLINE][1] * discount_rate_mid) / (len(user_buy_cnt_with_coupon_dict[user_id][ONLINE])+ 1); feature_idx += 1 # + 1 是为了防止除 0
        user_features_matrix[index, feature_idx] =  \
            np.sum(user_coupon_fraction_dict[user_id][OFFLINE][1] * discount_rate_mid) / (user_features_matrix[index, 3]+ 1); feature_idx += 1

        # 用户使用优惠券的最高/最低折扣率
        user_features_matrix[index, feature_idx] = user_coupon_fraction_dict[user_id][ONLINE][2]; feature_idx += 1
        user_features_matrix[index, feature_idx] = user_coupon_fraction_dict[user_id][ONLINE][3]; feature_idx += 1
        user_features_matrix[index, feature_idx] = user_coupon_fraction_dict[user_id][OFFLINE][2]; feature_idx += 1
        user_features_matrix[index, feature_idx] = user_coupon_fraction_dict[user_id][OFFLINE][3]; feature_idx += 1

        # 用户在各个商家使用的优惠券的数量, 使用的优惠券的数量占领取的比例以及占该用户使用优惠券的比例
        if (m_id in user_coupon_of_merchant_dict[user_id]):
            user_features_matrix[index, feature_idx] = user_coupon_of_merchant_dict[user_id][m_id][ONLINE][1]; feature_idx += 1
            user_features_matrix[index, feature_idx] = user_coupon_of_merchant_dict[user_id][m_id][OFFLINE][1]; feature_idx += 1
            # 使用的,未使用的优惠券的数量占在该商家领取的比例
            user_total_get_c_from_m = user_coupon_of_merchant_dict[user_id][m_id][ONLINE][0] + user_coupon_of_merchant_dict[user_id][m_id][OFFLINE][0]
            user_total_use_c_of_m = user_coupon_of_merchant_dict[user_id][m_id][ONLINE][1] + user_coupon_of_merchant_dict[user_id][m_id][OFFLINE][1]
            user_features_matrix[index, feature_idx] = user_total_use_c_of_m / (user_total_get_c_from_m + 1); feature_idx += 1
            user_features_matrix[index, feature_idx] = 1 - user_total_use_c_of_m / (user_total_get_c_from_m + 1); feature_idx += 1
            # 占该用户 总的使用优惠券的比例
            user_features_matrix[index, feature_idx] = user_coupon_of_merchant_dict[user_id][m_id][ONLINE][1] / \
                                                       (len(user_buy_cnt_with_coupon_dict[user_id][ONLINE]) + 1); feature_idx += 1
            user_features_matrix[index, feature_idx] = user_coupon_of_merchant_dict[user_id][m_id][OFFLINE][1] / \
                                                       (len(user_buy_cnt_with_coupon_dict[user_id][OFFLINE]) + 1); feature_idx += 1
        else:
            feature_idx += 6 # 没有feature 填 0， 这样特征数可以保持一致

        # 用户总共消费的次数（包括使用/未使用优惠券）
        user_features_matrix[:, feature_idx] = user_get_coupon_cnt_dict[user_id][ONLINE] + len(user_buy_cnt_with_coupon_dict[user_id][ONLINE]); feature_idx += 1 
        user_features_matrix[:, feature_idx] = user_get_coupon_cnt_dict[user_id][OFFLINE] + len(user_buy_cnt_with_coupon_dict[user_id][OFFLINE]); feature_idx += 1

        user_total_buy_cnt_online = user_buy_cnt_without_coupon_dict[user_id][ONLINE] + len(user_buy_cnt_with_coupon_dict[user_id][ONLINE])
        user_total_buy_cnt_offline = user_buy_cnt_without_coupon_dict[user_id][OFFLINE] + len(user_buy_cnt_with_coupon_dict[user_id][OFFLINE])

        # 用户使用优惠券消费的次数占消费次数的比例
        user_features_matrix[:, feature_idx] = len(user_buy_cnt_with_coupon_dict[user_id][ONLINE]) / (user_total_buy_cnt_online + 1) ; feature_idx += 1
        user_features_matrix[:, feature_idx] = len(user_buy_cnt_with_coupon_dict[user_id][OFFLINE]) / (user_total_buy_cnt_offline + 1) ; feature_idx += 1

        # 用户没有使用优惠券消费的次数占消费次数的比例
        user_features_matrix[:, feature_idx] = user_buy_cnt_without_coupon_dict[user_id][ONLINE] / (user_total_buy_cnt_online + 1) ; feature_idx += 1
        user_features_matrix[:, feature_idx] = user_buy_cnt_without_coupon_dict[user_id][OFFLINE] / (user_total_buy_cnt_offline + 1) ; feature_idx += 1

        # 用户使用优惠券消费的次数与没有使用次数的比值
        user_features_matrix[:, feature_idx] = len(user_buy_cnt_with_coupon_dict[user_id][ONLINE]) / \
                                               (user_buy_cnt_without_coupon_dict[user_id][ONLINE] + 1); feature_idx += 1 
        user_features_matrix[:, feature_idx] = len(user_buy_cnt_with_coupon_dict[user_id][OFFLINE]) / \
                                               (user_buy_cnt_without_coupon_dict[user_id][OFFLINE] + 1); feature_idx += 1
        # 用户线下使用优惠券距离商家的平均/最大/最小距离
        if (user_id in user_distance_dict):
            user_features_matrix[:, feature_idx] = len(user_distance_dict[user_id][0])/(sum(user_distance_dict[user_id][0]) + 1); feature_idx += 1
            user_features_matrix[:, feature_idx] = user_distance_dict[user_id][1]; feature_idx += 1
            user_features_matrix[:, feature_idx] = user_distance_dict[user_id][2]; feature_idx += 1
        else:
            feature_idx += 3 # 没有feature 填 0， 这样特征数可以保持一致
        
        # 用户线上操作次数, 及各操作的比例
        if (user_id in user_opt_online_dict):
            user_opt_cnt_online = sum(user_opt_online_dict[user_id]) + 1
            user_features_matrix[:, feature_idx : feature_idx + 3] = user_opt_cnt_online; feature_idx += 3
            user_features_matrix[:, feature_idx] = user_opt_online_dict[user_id][0] / user_opt_cnt_online; feature_idx += 1
            user_features_matrix[:, feature_idx] = user_opt_online_dict[user_id][1] / user_opt_cnt_online; feature_idx += 1
        ...
        # 用户在一周内的第几天使用过几次优惠券
        user_features_matrix[:, feature_idx : feature_idx + 7] = user_coupon_weekday_dict[user_id][ONLINE]; feature_idx += 7
        user_features_matrix[:, feature_idx : feature_idx + 7] = user_coupon_weekday_dict[user_id][OFFLINE]; feature_idx += 7 
    ...
    print("%s User feature matrix (%d, %d)" % (getCurrentTime(), sample_cnt, feature_idx))
    return user_features_matrix[:, 0:feature_idx]
